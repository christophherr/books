...
add_action( 'genesis_entry_header', 'ch_featured_image', 7 );
/**
 * Display linked featured image in entry header above the title.
 *
 * @return void
 */
function ch_featured_image() {
	if ( $image = genesis_get_image( 'format=url&size=posts-page' ) ) {
		printf( '<a href="%s"><img src="%s" alt="%s" class="aligncenter" /></a>', get_the_permalink(), $image, the_title_attribute( 'echo=0' ) );
	}
}
